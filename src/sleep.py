...
def sleep_phase(model, tokenizer, device, config, lifetime_elite_mask, prototype_memory, previous_mask, prototype_loader=None):
    """
    ...
    # ✅ [Pre-NREM 第一次刷新] 用当前任务数据更新原型，目的：获取正确的类别总数
    # 此处捕获的是 pre-NREM 特征（会被 NREM 后第二次刷新覆盖），只有 COUNT 被 NREM 使用
    # 关键：修复 nrem_num_classes=0 的 Bug（Task 0 时原本库为空，导致动态天花板 2.97 过小）
    ...
    with torch.no_grad():
        for name, param in model.named_parameters():
            ...
            if param.dim() > 1:
                ...
                dynamic_target_norm = base_target_norm * math.sqrt(max(1, nrem_num_classes) / 15.0)
    ...
    # ==========================================
    # 【坐标系修复】NREM 后立即刷新原型 feature（第二次刷新）
    # ==========================================
    # 第一次（Pre-NREM）：只为获取类别数，feature 在 pre-NREM 坐标系
    # 第二次（Post-NREM）：用 post-NREM 模型重新提取 feature，覆盖第一次的旧值
    # 效果：REM 使用的高斯样本与真实 post-NREM 特征空间一致，消除分布不匹配
    if prototype_loader is not None and prototype_memory is not None:
        print("   🔄 [Post-NREM] 刷新原型 feature（第二次，对齐 post-NREM 特征空间）...")
        prototype_memory.update_prototypes(model, prototype_loader, device)

    # REM 用的类别数（刷新后应与 nrem_num_classes 相同）
    num_classes = len(prototype_memory.prototypes) if prototype_memory else 0
    ...
    if params_to_opt:
        ...
        dream_cycles = max(800, num_classes * 30)
